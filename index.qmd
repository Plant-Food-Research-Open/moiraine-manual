...
* Integration of the omics datasets by one or more of the supported tools (see below); currently, the following integration methods are covered in `moiraine`:
...
Note that for readers that prefer to use R scripts to `targets` pipeline, the target command in the example target chunk above can be converted to R script-compatible code as follows:
...
Throughout the manual, the following options are set to improve upon the default colour scales:
